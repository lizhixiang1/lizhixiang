...
public final class GameServer {
    ...
    */
    public GameServer(File entitiesFile, File actionsFile) {
        new DispatchCenter(entitiesFile,actionsFile);
    }
    ...
    */
    public String handleCommand(String command) {
        ...
        String commands=split[1].trim();
        String[] commandTokens=commands.split(" ");
        ActionParser actionParser=new ActionParser();
        GameAction action = actionParser.parserAction(commandTokens, commands);
        if(action==null){
            return "Something wrong!";
        }
        ActionCommand actionCommand = new ActionCommand();
        return actionCommand.execute(action, userName);
    }
